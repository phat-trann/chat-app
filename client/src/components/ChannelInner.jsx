...
import { Circle, Delete, Info } from '@mui/icons-material';
          ...
          {(channel.type === 'messaging') ?
                  ...
                  <Grid item xs={3} key={i}>
                    <Grid container sx={{ alignItems: 'center', justifyContent: 'center' }}>
                      <Grid item xs={12} md={4}>
                        {
                          (user && i <= 3) ?
                            ((i < 3) ? (<IconButton sx={{ '& .str-chat__avatar': { margin: 0 } }}>
            ...
            (
              <Grid container sx={{ alignItems: 'center', justifyContent: 'center', pl: 0, pointerEvents: 'all' }}>
                ...
                <Grid item xs={10} sx={{ width: '100%', textAlign: 'center' }}># {channel.data.name}</Grid>
              </Grid>
            )
          }
          ...
          <Grid item xs={12} md={8}>
            <Container sx={{ pt: 1.5, pb: 1, background: '#ededed' }}>
              <Grid container sx={{ alignItems: 'center', pl: 0, pointerEvents: 'all' }}>
                ...
                <Grid item xs={7} md={8} sx={{ '& div': { cursor: 'pointer' } }}>
                  <MessagingHeader />
                </Grid>
                <Grid item xs={2}>
                </Grid>
                <Grid item xs={1}>
                  <IconButton onClick={() => { setOpen(true) }} sx={{ bgcolor: 'background.paper' }}>
                    <Delete />
                  </IconButton>
                </Grid>
              </Grid>
