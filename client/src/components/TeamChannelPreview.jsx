...
import { HIDE_MENU, RESET_STATUS } from '../actions/types';
...
const TeamChannelPreview = ({ channel, type }) => {
  ...
  const DirectPreview = () => {
    ...
    return (
      <>
        <ListItemIcon>
          <Avatar image={members[0]?.user?.image} name={members[0]?.user?.name} size={30} />
        </ListItemIcon>
        <ListItemText
          primary={`${members[0]?.user?.name || members[0]?.user?.id}${
            members[1]?.user?.name ? ', ...' : ''
          }`}
        />
      </>
